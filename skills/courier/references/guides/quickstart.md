...
| `status: UNROUTABLE` / `PROVIDER_ERROR`, `"No provider(s) courier ... : undefined"` | The template has no routing strategy attached. `POST /notifications` does not persist `routing`, so API-created templates start with none. | Attach a strategy via `PUT /notifications/{id}`, see [inbox.md](../channels/inbox.md#troubleshooting). |
